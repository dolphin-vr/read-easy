...
<div><p class="shopping-book-name">${book.title}</p><p class="shopping-book-category"loading="lazy">${book.list_name}</p>
...
function bookShopsMurkup(shops) {
  ...
  shopIcons.forEach(el => {
    markup += `<li class="shop"><a href="${el.url}" class="shop-link" target="_blank" rel="noreferrer noopener"><img srcset="${el.path2x} 2x" src="${el.path}" alt="${el.name}" class="shop-icon ${el.name}"loading="lazy"/></a></li>`;
  });
  markup += '</ul>';
